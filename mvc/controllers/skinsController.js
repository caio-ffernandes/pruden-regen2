...
module.exports = (app) => {
    ...
    app.get("/skins", async (req, res) => {
        ...
        res.render("skin/addskins", { promocoes: lista_promocoes })
        //Retorna os dados
     ...
     app.post("/registrarskin", upload.fields([{ name: 'photo1', maxCount: 1 }, { name: 'photo2', maxCount: 1 }]), async (req, res) => {
        ...
        try {
            ...
            const status = await skinDAO.registrarSkin(selcatskin, txtnomeskin, txtdescskin, selgenskin, txtvalorskin, selrarskin, nomeArquivo1SemPath, nomeArquivo2SemPath, selproskin);
     ...
     app.get("/pagina/skin", (req, res) => {
         ...
         res.sendFile(path.resolve("mvc/views/ctrldev/skin/addskins.html"))
     })
    ...
    app.get("/skin/lista", async (req, res) => {
        res.setHeader("Access-Control-Allow-Origin","*")
        const skinDAO = new SkinDAO()
        const lista_skins = await skinDAO.consultarSkins()
        res.render("skin/listskins", { skins: lista_skins })
    })
    app.get("/skin/alterar/:id", async (req, res) => {
        ...
        res.render("skin/upskins", { skin: dtskin  })
    })
    app.get("/skin/bruno", async (req, res) => {
        const skinDAO = new SkinDAO()
        const lista_skins = await skinDAO.consultarSkins()

        res.render("../loja/SkinsN", { skins: lista_skins  })
    })
    app.get("/skin/brunaldo/:id", async (req, res) => {
        ...
        res.render("../loja/detalhes", { skins: dtskin  })
    })
